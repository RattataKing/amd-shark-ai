...
def scan_directory(base_dir, threshold):
    ...
    plt.title(f"Compilation Rate Distribution in {Path(base_dir).name}")

    out_path = os.path.join(base_dir, "compilation_rate.png")
    plt.tight_layout()
...
def main():
    for dest_dir in DEST_DIRS:
        if not os.path.isdir(dest_dir):
            ...
            continue
        out_path = scan_directory(dest_dir, THRESHOLD)
